<script setup>
import { DotIcon, BinocularsIcon } from "lucide-vue-next";

const { row, class: classes } = defineProps(["row", "class"]);
const domains = ref(row.original?.domain.split(" "));
const hasValidSSL = computed(() => row.original?.enableSSL && row.original?.hasSSL);
</script>

<template>
  <div :class="cn('hstack', { '**:text-gray-500 opacity-40': !row?.original?.isActive }, classes)">
