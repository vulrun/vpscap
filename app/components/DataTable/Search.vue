...
<template>
  <div>
    <Button variant="outline" size="sm" class="relative h-10 w-full justify-start bg-background text-muted-foreground sm:w-32 md:w-48" @click="isOpen = true">
    ...
    <Dialog :open="isOpen" @update:open="isOpen = !isOpen">
      <DialogContent class="overflow-hidden p-0 shadow-lg">
        ...
        >
          ...
          <CommandList>
            <CommandEmpty v-if="searchTerm">No links found.</CommandEmpty>
              ...
              <CommandItem v-for="link in filteredLinks" :key="link.item?.id" class="cursor-pointer" :value="link.item" @select="selectLink(link.item)">
                <div class="flex gap-1 w-full">
                  <div class="flex-1 overflow-hidden inline-flex gap-1 items-center">
                    ...
                    <div class="text-xs text-muted-foreground flex-1 truncate">({{ link.item?.url }})</div>
                  </div>
