...
module.exports = class NginxParser {
  constructor() {
    // Last used file name
    this.fileName = null;
  ...
  setFileName(fileName) {
    ...
    if (this.serverRoot === null) {
      this.serverRoot = fsPath.dirname(fileName);
    }
  }
      ...
      */
      if (line.endsWith("{")) {
        ...
        */
        ...
        const findFiles = fsPath.resolve(this.serverRoot || options.includesRoot || "", line.replace("include ", "").replace(";", "").trim());
...
// const raw = fs.readFileSync(process.env.PWD + "/templates/nginx-vhost.txt") + "";
...
// const splitBraces = (str) => {
//     let matches = String(str).match(/([^\{]+)\{(.*)\}/);
//     matches = matches.map((i) => i.trim());
//     return [matches[1], matches[2]];
// };

// const splitSpaces = (str) => {
//     const matches = String(str).split(/\;\s/);
//     // matches = matches.map((i) => i.trim());
//     return matches;
// };
...
//     if (/\{(.*)\}/.test(str)) {
//         const [key, val] = splitBraces(str);
//         parsed[key] = val || splitSpaces(val);
//     }
