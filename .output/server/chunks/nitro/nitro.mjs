...
import fs__default, { promises, existsSync } from 'node:fs';
import * as fsPath$1 from 'node:path';
import fsPath__default, { resolve as resolve$1, dirname as dirname$1, join } from 'node:path';
...
const _inlineRuntimeConfig = {
  "app": {
    "baseURL": "/",
    "buildId": "6159790e-6354-44a9-9398-4369d049c84c",
...
function hexDecode(hexString) {
  const hexInput = String(hexString != null ? hexString : "").replace(/\r|\n/g, "");
  if (!hexInput) return "";
...
class JsonHandler {
  constructor({ cwd, dir, file }) {
    this.cwdPath = cwd ? fsPath__default.resolve(cwd) : fsPath__default.resolve(".");
    this.dirPath = fsPath__default.resolve(`${this.cwdPath}/${dir}/`);
    this.filePath = fsPath__default.resolve(`${this.dirPath}/${file}.json`);
  }
  touch() {
    if (fs__default.existsSync(this.dirPath) === false) {
      fs__default.mkdirSync(this.dirPath);
    }
    if (fs__default.existsSync(this.filePath) === false) {
      fs__default.writeFileSync(this.filePath, "{}");
    }
  }
  write(data) {
    const content = this.readable === true ? JSON.stringify(data, null, 2) : JSON.stringify(data);
    return fs__default.writeFileSync(this.filePath, content);
  }
  read() {
    try {
      return JSON.parse(fs__default.readFileSync(this.filePath, "utf8"));
    } catch (error) {
      return null;
    }
  }
...
class JsonDbExtended extends JsonDb {
  constructor(...args) {
    var _a;
    super(...args);
...
const _SslMeta = class _SslMeta {
  constructor() {
    __privateAdd$3(this, _SslMeta_instances);
    if (_SslMeta.instance) {
      return _SslMeta.instance;
    }
  ...
  async fetch(domain) {
    ...
    try {
      ...
      if (cached && !(cached == null ? void 0 : cached.error)) return cached;
      const options = {
        host: domain,
        port: 443,
        method: "GET",
      ...
      const resp = await __privateMethod$3(this, _SslMeta_instances, httpsRequest_fn).call(this, options);
      ...
      cert.validNames = cert.subject_names.filter((n) => __privateMethod$3(this, _SslMeta_instances, isWildcardMatch_fn).call(this, domain, n));
      ...
      cert.days_left = __privateMethod$3(this, _SslMeta_instances, daysLeft_fn$1).call(this, peerCert == null ? void 0 : peerCert.valid_to);
      cert.isExpired = cert.days_left <= 0;
      cert.isValid = (cert == null ? void 0 : cert.validNames.length) > 0 && cert.days_left > 0;
      cert.remarks = __privateMethod$3(this, _SslMeta_instances, remarks_fn).call(this, cert);
  ...
  fetchAllCached(domains) {
    ...
    const promises = domains.map((d) => {
      ...
      if (data == null ? void 0 : data.remarks) return data;
  ...
  async getCertsExpiringIn(dayCount) {
    ...
    const expiring = monitored.filter((itm) => (itm == null ? void 0 : itm.days_left) <= (dayCount || -1));
    const sanitized = expiring.filter((itm) => itm.validNames.length);
    const validated = sanitized.map((itm) => {
      if ((itm == null ? void 0 : itm.validNames.length) < 1) throw new Error("Domain is missing");
      if ((itm == null ? void 0 : itm.validNames.length) !== 1) throw new Error("Mulitple domains are not allowed");
      return itm;
    });
    return validated.sort((a, b) => (a == null ? void 0 : a.days_left) - (b == null ? void 0 : b.days_left));
  }
};
...
const require$$0 = /*@__PURE__*/getDefaultExportFromNamespaceIfNotNamed(fsPath$1);

const require$$1 = /*@__PURE__*/getDefaultExportFromNamespaceIfNotNamed(fs$1);

const fsPath = require$$0;
const fs = require$$1;
...
const locateFile = (_filename, _options = {}) => {
  ...
  const basePath = fsPath.resolve((options == null ? void 0 : options.startDir) || ".");
  ...
  const indexRange = options.reverse ? Array.from({ length: segments.length - root }, (_, i) => i + root + 1) : Array.from({ length: segments.length - root }, (_, i) => segments.length - i);
  for (const index of indexRange) {
    for (const filename of filenames) {
      ...
      if (options.validate(filePath)) {
        return (options == null ? void 0 : options.returns) === "dirPath" ? dirPath : filePath;
      }
    }
  }
  return null;
...
const locatePath$1 = /*@__PURE__*/getDefaultExportFromCjs(locatePath);
...
class AccountJson {
  constructor() {
    ...
    const vpscapRootPath = locatePath$1.nearestDirPath(".git/config");
    this.dirPath = fsPath__default.resolve(vpscapRootPath, ".localdb");
    this.filePath = fsPath__default.resolve(vpscapRootPath, ".localdb", "account.json");
    if (!fs$2.existsSync(this.filePath)) {
      fs$2.writeFileSync(this.filePath, JSON.stringify({}, null, 2), "utf8");
    }
    AccountJson.instance = this;
  }
  readJson() {
    try {
      const data = fs$2.readFileSync(this.filePath, "utf8");
  ...
  writeJson(data) {
    fs$2.writeFileSync(this.filePath, JSON.stringify(data, null, 2), "utf8");
  }
  ...
  getData(keys) {
    ...
    if (Array.isArray(keys)) {
      const result = {};
      for (const key of keys) {
        result[key] = (_a = data == null ? void 0 : data[key]) != null ? _a : null;
      }
      return result;
    }
...
async function sendEmailNow({ to, subject, body }) {
  try {
    ...
    if (!(config == null ? void 0 : config.smtpUrl)) throw new Error("`smtpUrl` is missing for sendEmailNow.");
    if (!(config == null ? void 0 : config.smtpFrom)) throw new Error("`smtpFrom` is missing for sendEmailNow.");
    const transporter = nodemailer.createTransport(config == null ? void 0 : config.smtpUrl);
    await transporter.verify();
    const mailOptions = {};
    mailOptions.from = config == null ? void 0 : config.smtpFrom;
    mailOptions.to = to;
    mailOptions.subject = subject;
    mailOptions.headers = {
      "List-Help": `<mailto:${config == null ? void 0 : config.smtpFrom}?subject=NEED-HELP>`,
      "List-Unsubscribe": `<mailto:${config == null ? void 0 : config.smtpFrom}?subject=UNSUBSCRIBE>`
    };
    ...
    console.log("\u{1F680} ~ sendEmailNow ~ mailResp:", mailResp);
    const sentInfo = {};
    sentInfo.response = String((mailResp == null ? void 0 : mailResp.response) || "").toUpperCase();
    sentInfo.success = mailResp == null ? void 0 : mailResp.response.includes("OK");
    sentInfo.remarks = `Email successfully sent to \`${mailResp == null ? void 0 : mailResp.accepted.join(", ")}\`.`;
    ...
    return {
      success: false,
      remarks: `Failed to send email. ${err == null ? void 0 : err.message}`,
      message: err == null ? void 0 : err.message,
      response: err == null ? void 0 : err.message,
      __raw: null
    };
  }
}
async function getAlertsHtml(context) {
  Handlebars.registerHelper("ternary", function(test, yes, no) {
    return test ? yes : no;
  });
  const templatePath = fsPath__default.join(process.cwd(), "server/utils/raw/ssl-alert-template.html");
  const templateRaw = fs$2.readFileSync(templatePath, "utf-8");
  ...
  return minify(templateHtml, {
    ...
    minifyCSS: true,
    // Inline CSS gets minified
    minifyJS: true,
    // Inline JS gets minified (if any, rare in emails)
    // useShortDoctype: true,
    keepClosingSlash: true,
    // Important for self-closing tags in XHTML
    removeAttributeQuotes: false
    // Keep quotes for better email client compatibility
  });
}
async function runCronJobTask(jobSlug, runForcefully) {
  var _a;
  ...
  if (!runForcefully && !((_a = accObj == null ? void 0 : accObj.cronJobSettings) == null ? void 0 : _a[jobSlug])) {
  ...
  switch (jobSlug) {
    ...
    case "installed_certs_daily_alerts": {
      ...
      const generatedAt = (/* @__PURE__ */ new Date()).toISOString().split(".")[0].replace("T", " ");
      const dashboardUrl = `${accObj == null ? void 0 : accObj.homeUrl}/#settings`;
      const unsubscribeUrl = `${accObj == null ? void 0 : accObj.homeUrl}/#settings/notifications`;
      const certs = expiringCerts.map((crt) => {
        var _a2, _b, _c;
        return {
          notes: crt == null ? void 0 : crt.domains.filter((c) => c !== (crt == null ? void 0 : crt.domain)).join(", "),
          domain: crt == null ? void 0 : crt.domain,
          issuer: `${(_a2 = crt == null ? void 0 : crt.issuedBy) == null ? void 0 : _a2.organizationName}, ${(_b = crt == null ? void 0 : crt.issuedBy) == null ? void 0 : _b.countryName}`,
          expiresOn: `${(_c = (crt == null ? void 0 : crt.expiresAtIso) || "") == null ? void 0 : _c.substring(0, 10)}`,
          daysLeft: (crt == null ? void 0 : crt.daysLeft) == 1 ? `${crt == null ? void 0 : crt.daysLeft} day` : (crt == null ? void 0 : crt.daysLeft) > 0 ? `${crt == null ? void 0 : crt.daysLeft} days` : `EXPIRED`,
          badgeClass: (crt == null ? void 0 : crt.daysLeft) <= 1 ? `badge-red` : (crt == null ? void 0 : crt.daysLeft) <= 3 ? `badge-orange` : (crt == null ? void 0 : crt.daysLeft) <= 7 ? `badge-yellow` : `badge-gray`
        };
      });
      ...
      const sentInfo = await sendEmailNow({
        to: accObj == null ? void 0 : accObj.loginMail,
        subject,
        body: html
      });
      return sentInfo;
    ...
    case "monitored_certs_daily_alerts": {
      ...
      const certs = expiringCerts.map((crt) => {
        var _a2;
        return {
          notes: crt == null ? void 0 : crt.subject_alt_name.filter((c) => c !== (crt == null ? void 0 : crt.domain)).join(", "),
          domain: crt == null ? void 0 : crt.domain,
          issuer: `${crt == null ? void 0 : crt.issuer_org}, ${crt == null ? void 0 : crt.issuer_loc}`,
          expiresOn: `${(_a2 = (crt == null ? void 0 : crt.expiry) || "") == null ? void 0 : _a2.substring(0, 10)}`,
          daysLeft: (crt == null ? void 0 : crt.days_left) == 1 ? `${crt == null ? void 0 : crt.days_left} day` : (crt == null ? void 0 : crt.days_left) > 0 ? `${crt == null ? void 0 : crt.days_left} days` : `EXPIRED`,
          badgeClass: (crt == null ? void 0 : crt.days_left) <= 1 ? `badge-red` : (crt == null ? void 0 : crt.days_left) <= 3 ? `badge-orange` : (crt == null ? void 0 : crt.days_left) <= 7 ? `badge-yellow` : `badge-gray`
        };
      });
      const html = await getAlertsHtml({
        subject,
...
function installedPath(pkgName) {
  try {
...
function isInstalled(pkgName) {
  return installedPath(pkgName).then((i) => true).catch((e) => false);
}
async function getTracedARecords(domain, depth = 0) {
  if (depth > 10) return [];
...
var _client, _certsRootPath, _certsLivePath, _certsTrashPath, _challengesPath$1, _accountKeyPath, _accountObjPath, _sslDhParamsPath, _sslOptionsNginx, _SslAcme_instances, setupDirectories_fn, generateOptionsSslNginxConf_fn, generateDhParams_fn, getOrCreateAccountKey_fn, generateCsr_fn, saveCertificate_fn, handleHttpChallenge_fn, cleanupHttpChallenge_fn, createPemContent_fn, isCertKeyFile_fn, isCertMainFile_fn;
const _SslAcme = class _SslAcme {
  constructor(args) {
    __privateAdd$2(this, _SslAcme_instances);
    __privateAdd$2(this, _client);
    __privateAdd$2(this, _certsRootPath);
    __privateAdd$2(this, _certsLivePath);
    __privateAdd$2(this, _certsTrashPath);
    __privateAdd$2(this, _challengesPath$1);
    __privateAdd$2(this, _accountKeyPath);
    __privateAdd$2(this, _accountObjPath);
    __privateAdd$2(this, _sslDhParamsPath);
    __privateAdd$2(this, _sslOptionsNginx);
    if (_SslAcme.instance) {
      return _SslAcme.instance;
    }
    const accJson = new AccountJson();
    __privateSet$1(this, _certsRootPath, fsPath__default.resolve(accJson.dirPath, "acme-ssl"));
    __privateSet$1(this, _certsLivePath, fsPath__default.resolve(accJson.dirPath, "acme-ssl", "certs"));
    __privateSet$1(this, _certsTrashPath, fsPath__default.resolve(accJson.dirPath, "acme-ssl", "trashed-certs"));
    __privateSet$1(this, _challengesPath$1, fsPath__default.resolve(accJson.dirPath, "acme-ssl", "challenges"));
    __privateSet$1(this, _accountKeyPath, fsPath__default.resolve(accJson.dirPath, "acme-ssl", "account-key.pem"));
    __privateSet$1(this, _accountObjPath, fsPath__default.resolve(accJson.dirPath, "acme-ssl", "account-acme.json"));
    __privateSet$1(this, _sslDhParamsPath, fsPath__default.resolve(accJson.dirPath, "acme-ssl", "ssl-dhparams.pem"));
    __privateSet$1(this, _sslOptionsNginx, fsPath__default.resolve(accJson.dirPath, "acme-ssl", "options-ssl-nginx.conf"));
    ...
    __privateMethod$2(this, _SslAcme_instances, setupDirectories_fn).call(this);
    _SslAcme.instance = this;
  }
  async listCertificates() {
    var _a, _b, _c, _d, _e, _f, _g, _h, _i, _j, _k, _l, _m;
    if (!shelljs.test("-d", __privateGet$1(this, _certsLivePath))) throw new Error("Invalid Path");
    const certsDomain = fs$2.readdirSync(__privateGet$1(this, _certsLivePath));
    const data = [];
    for (const domain of certsDomain) {
      const certDir = fsPath__default.resolve(__privateGet$1(this, _certsLivePath), domain);
      const stat = fs$2.statSync(certDir);
      if (!stat.isDirectory()) continue;
      const certFiles = fs$2.readdirSync(certDir).map((path) => fsPath__default.resolve(certDir, path));
      const certMainFile = certFiles.find((file) => __privateMethod$2(this, _SslAcme_instances, isCertMainFile_fn).call(this, file));
      const certKeyFile = certFiles.find((file) => __privateMethod$2(this, _SslAcme_instances, isCertKeyFile_fn).call(this, file));
      const certData = fs$2.readFileSync(certMainFile, "utf8");
      ...
      extendObj(certInfo, {
        ...
        "_parsed.subject.commonName": (_a = toObject(certInfo == null ? void 0 : certInfo.subject.attributes, "name", "value")) == null ? void 0 : _a.commonName,
      ...
      tmpData.altNames = cleanArray(typeof ((_f = _parsed == null ? void 0 : _parsed.subject) == null ? void 0 : _f.altNames) === "string" ? (_h = (_g = _parsed == null ? void 0 : _parsed.subject) == null ? void 0 : _g.altNames) == null ? void 0 : _h.split(" ") : (_i = _parsed == null ? void 0 : _parsed.subject) == null ? void 0 : _i.altNames);
      ...
      tmpData.issuedAtIso = new Date((_k = certInfo == null ? void 0 : certInfo.validity) == null ? void 0 : _k.notBefore).toISOString();
      tmpData.expiresAt = new Date((_l = certInfo == null ? void 0 : certInfo.validity) == null ? void 0 : _l.notAfter).valueOf();
      tmpData.expiresAtIso = new Date((_m = certInfo == null ? void 0 : certInfo.validity) == null ? void 0 : _m.notAfter).toISOString();
      data.push(tmpData);
    }
    return data;
  }
  getPaths() {
    return {
      certsRootPath: __privateGet$1(this, _certsRootPath),
      certsLivePath: __privateGet$1(this, _certsLivePath),
      certsTrashPath: __privateGet$1(this, _certsTrashPath),
      challengesPath: __privateGet$1(this, _challengesPath$1),
      accountKeyPath: __privateGet$1(this, _accountKeyPath),
      accountObjPath: __privateGet$1(this, _accountObjPath),
      sslDhParamsPath: __privateGet$1(this, _sslDhParamsPath),
      sslOptionsNginx: __privateGet$1(this, _sslOptionsNginx)
    };
  }
  async initialize() {
    const accountKey = await __privateMethod$2(this, _SslAcme_instances, getOrCreateAccountKey_fn).call(this);
    __privateSet$1(this, _client, new acme.Client({
      accountKey,
  ...
  async issueCertificate(domain) {
    try {
      ...
      const csrData = await __privateMethod$2(this, _SslAcme_instances, generateCsr_fn).call(this, domain);
      console.log("\u{1F50F} Requesting CA for certificates...");
      const cert = await __privateGet$1(this, _client).auto({
        csr: csrData == null ? void 0 : csrData.csr,
        email: this.email,
        termsOfServiceAgreed: true,
        challengeCreateFn: __privateMethod$2(this, _SslAcme_instances, handleHttpChallenge_fn).bind(this),
        challengeRemoveFn: __privateMethod$2(this, _SslAcme_instances, cleanupHttpChallenge_fn).bind(this)
      });
      await __privateMethod$2(this, _SslAcme_instances, saveCertificate_fn).call(this, domain, cert, csrData == null ? void 0 : csrData.csr, csrData == null ? void 0 : csrData.key);
  ...
  async trashCertificate(domain) {
    const certLiveDomainPath = fsPath__default.resolve(__privateGet$1(this, _certsLivePath), domain);
    const certTrashDomainPath = fsPath__default.resolve(__privateGet$1(this, _certsTrashPath), domain);
    const dirExists = await fs$2.access(certLiveDomainPath, fs$2.constants.R_OK | fs$2.constants.W_OK).then(() => true).catch(() => false);
    if (!dirExists) throw new Error(`Certifcate not found for [${domain}]`);
    await fs$2.move(certLiveDomainPath, certTrashDomainPath);
  }
};
_client = new WeakMap();
...
setupDirectories_fn = function() {
  fs$2.ensureDirSync(__privateGet$1(this, _certsLivePath));
  fs$2.ensureDirSync(__privateGet$1(this, _certsTrashPath));
  fs$2.ensureDirSync(__privateGet$1(this, _challengesPath$1));
  __privateMethod$2(this, _SslAcme_instances, generateOptionsSslNginxConf_fn).call(this);
  __privateMethod$2(this, _SslAcme_instances, generateDhParams_fn).call(this);
};
generateOptionsSslNginxConf_fn = async function() {
  ...
  fs$2.writeFileSync(__privateGet$1(this, _sslOptionsNginx), Buffer.from(optionsSslNginxConf, "base64"));
};
generateDhParams_fn = async function() {
  if (fs$2.existsSync(__privateGet$1(this, _sslDhParamsPath))) return;
  ...
  fs$2.writeFileSync(__privateGet$1(this, _sslDhParamsPath), __privateMethod$2(this, _SslAcme_instances, createPemContent_fn).call(this, prime, "DH PARAMETERS"));
  console.log("DH Parameters saved to dhparam.pem", "Generator:", generator.toString());
};
getOrCreateAccountKey_fn = async function() {
  if (!fs$2.existsSync(__privateGet$1(this, _accountKeyPath))) {
    console.log("\u{1F511} Generating new account key...");
    const accountKey = await acme.crypto.createPrivateEcdsaKey();
    await fs$2.outputFile(__privateGet$1(this, _accountKeyPath), accountKey);
  }
  return fs$2.readFileSync(__privateGet$1(this, _accountKeyPath), "utf8");
};
...
saveCertificate_fn = async function(domain, cert, csr, key) {
  var _a;
  if (!domain) throw new Error("SSLManager: Domain is missing");
  const certPath = fsPath__default.resolve(__privateGet$1(this, _certsLivePath), String((_a = cleanArray(domain)) == null ? void 0 : _a[0]));
  await fs$2.ensureDir(certPath);
  await fs$2.outputFile(`${certPath}/cert.pem`, cert);
  await fs$2.outputFile(`${certPath}/cert.crt`, cert);
  await fs$2.outputFile(`${certPath}/priv.pem`, key);
  await fs$2.outputFile(`${certPath}/priv.key`, key);
  await fs$2.outputFile(`${certPath}/sign.pem`, csr);
  await fs$2.outputFile(`${certPath}/sign.csr`, csr);
...
handleHttpChallenge_fn = async function(authz, challenge, keyAuthorization) {
  const challengeFilePath = fsPath__default.join(__privateGet$1(this, _challengesPath$1), challenge == null ? void 0 : challenge.token);
  await fs$2.outputFile(challengeFilePath, keyAuthorization || "");
...
cleanupHttpChallenge_fn = async function(authz, challenge, keyAuthorization) {
  const challengeFilePath = fsPath__default.join(__privateGet$1(this, _challengesPath$1), challenge == null ? void 0 : challenge.token);
  await fs$2.remove(challengeFilePath);
...
isCertKeyFile_fn = function(filePath) {
  ...
  const pathInfo = fsPath__default.parse("" + filePath);
  const fileData = fs$2.readFileSync("" + filePath, "utf8");
...
class NginxParser {
  ...
  readConfigFile(fileName, cb, options) {
    ...
    if (cb) {
      fs__default.stat(fileName, (statsErr, stats) => {
        if (statsErr) return cb(statsErr, null);
        else if (!stats.isFile()) return cb(new ReferenceError("File does not exist"), null);
        fs__default.readFile(fileName, (readErr, configString) => {
          if (readErr) return cb(readErr, null);
          cb(null, this.parse(configString, options));
        });
      });
    } else {
      if (!fs__default.statSync(fileName).isFile()) throw new ReferenceError("File does not exist");
      const configString = fs__default.readFileSync(fileName);
  ...
  writeConfigFile(fileName, data, overwrite = false, cb = null) {
    ...
    if (cb) {
      fs__default.stat(fileName, (statsErr, stats) => {
        ...
        fs__default.writeFile(fileName, data, (writeErr) => {
          if (writeErr) return cb(writeErr, null);
          cb(null, true);
        });
      });
    } else {
      if (fs__default.statSync(fileName).isFile() && !overwrite) throw new Error("File already exists, to overwrite, set `overwrite = true`");
      if (typeof data === "object") data = this.toConf(data);
      return fs__default.writeFileSync(fileName, data) === void 0;
    }
  }
  /**
   * @method parse
...
var _challengesPath, _allowedKeys, _NginxHandler_instances, buildConfRootLocation_fn, fetchConfMeta_fn;
const _NginxHandler = class _NginxHandler {
  constructor(args) {
    __privateAdd$1(this, _NginxHandler_instances);
    __privateAdd$1(this, _challengesPath);
    __privateAdd$1(this, _allowedKeys, ["enableIndexing", "enableSSL", "forceSSL", "confType", "domain", "target"]);
    if (_NginxHandler.instance) {
      return _NginxHandler.instance;
    }
    const accJson = new AccountJson();
    __privateSet(this, _challengesPath, fsPath__default.resolve(accJson.dirPath, "acme-ssl", "challenges"));
    this.nginx = new NginxParser();
    this.webSites = args == null ? void 0 : args.webSites;
    _NginxHandler.instance = this;
  }
  ...
  async validateAndSanitize(input) {
    ...
    input.domain = sanitizeDomains(input == null ? void 0 : input.domain);
    ...
    const result = await ConfSchema.safeParseAsync(lo.pick(input, __privateGet(this, _allowedKeys)));
    if (result == null ? void 0 : result.success) return result == null ? void 0 : result.data;
    const firstError = (_b = (_a = result == null ? void 0 : result.error) == null ? void 0 : _a.issues) == null ? void 0 : _b[0];
  ...
  async touchConf(path) {
    if (fs$2.existsSync(path) === false) {
      fs$2.writeFileSync(path, "");
    }
  }
  async readConf(path) {
    var _a, _b, _c;
    ...
    try {
      ...
      jsonData.domain = cleanArray(jsonData == null ? void 0 : jsonData.domain).join(" ");
      return JSON.parse(JSON.stringify(jsonData));
    } catch (e) {
      const config = this.nginx.toJSON(raw);
      if (!(config == null ? void 0 : config.server)) return null;
      const domain = sanitizeDomains(((_a = config == null ? void 0 : config.server) == null ? void 0 : _a.server_name) || "").join(" ");
  ...
  async writeConf(path, obj) {
    var _a, _b, _c;
    obj = lo.pick(obj, __privateGet(this, _allowedKeys));
    const isDefaultConf = path.endsWith("_default.conf");
    const portSuffix = isDefaultConf ? "default_server" : "";
    obj = JSON.parse(JSON.stringify(obj));
    obj.domain = sanitizeDomains(obj == null ? void 0 : obj.domain);
    if (((_a = obj == null ? void 0 : obj.domain) == null ? void 0 : _a.length) <= 0) throw new Error("Domain is missing");
    ...
    const rootLocation = await __privateMethod$1(this, _NginxHandler_instances, buildConfRootLocation_fn).call(this, obj);
    ...
    locations.acmeChallenges = {
      "location /.well-known/acme-challenge/": {
        alias: __privateGet(this, _challengesPath) + "/"
      }
    };
    locations.forceSSL = {
      "location /": {
    ...
    if ((obj == null ? void 0 : obj.forceSSL) && (obj == null ? void 0 : obj.enableSSL)) {
      ...
      if ((obj == null ? void 0 : obj.confType) === "serve") Object.assign(server80, { root: obj == null ? void 0 : obj.target });
      Object.assign(server80, locations == null ? void 0 : locations.acmeChallenges);
      ...
      if ((obj == null ? void 0 : obj.confType) === "serve" && phpFpmSocket) Object.assign(server80, locations == null ? void 0 : locations.phpFpm);
    }
    const server443 = {};
    if (obj == null ? void 0 : obj.enableSSL) {
      ...
      if ((obj == null ? void 0 : obj.confType) === "serve") Object.assign(server443, { root: obj == null ? void 0 : obj.target });
      ...
      if ((obj == null ? void 0 : obj.confType) === "serve" && phpFpmSocket) Object.assign(server443, locations == null ? void 0 : locations.phpFpm);
  ...
  async restart() {
    try {
      ...
      console.log("\u274C nginx configuration restarted:", (err == null ? void 0 : err.message) || err);
    }
  }
};
...
buildConfRootLocation_fn = async function(obj) {
  ...
  if ((obj == null ? void 0 : obj.confType) === "redirect") {
    return { rewrite: [`^/$ ${obj == null ? void 0 : obj.target} redirect`, `^/(.*)$ ${obj == null ? void 0 : obj.target}/$1 redirect`] };
  }
...
let NginxHandler = _NginxHandler;
...
var _WebSites_instances, parseHexaId_fn, generateFilename_fn, setConfData_fn, newDefaultConf_fn, dnsIpLookup_fn, checkDomainsInUse_fn, filterCerts_fn, unwindCerts_fn, daysLeft_fn;
const _WebSites = class _WebSites {
  constructor() {
    __privateAdd(this, _WebSites_instances);
    if (_WebSites.instance) {
      return _WebSites.instance;
    }
    const accountJson = new AccountJson();
    const accObj = accountJson.getData("*");
    if (!(accObj == null ? void 0 : accObj.systemUser)) {
      console.log("Please setup admin user first");
      process.exit();
    }
    this.sslMeta = new SslMeta();
    this.sslAcme = new SslAcme({ webSites: this, email: accObj == null ? void 0 : accObj.loginMail });
    this.nginx = new NginxHandler({ webSites: this });
    this.nginxReload = debounce(() => this.nginx.reload(), 1e3);
    this.confDirPath = fsPath__default.resolve(accObj == null ? void 0 : accObj.localDir, "sites.conf.d");
    this.touch();
    _WebSites.instance = this;
  }
  touch() {
    if (fs$2.existsSync(this.confDirPath) === false) {
      fs$2.mkdirSync(this.confDirPath);
    }
  }
  async list(options) {
    var _a;
    if (!shelljs.test("-d", this.confDirPath)) throw new Error("Invalid Path");
    const confFiles = shelljs.find(this.confDirPath);
    const sslDomains = (options == null ? void 0 : options.minimal) ? [] : await this.findSslDomains();
    const sslMonitors = (options == null ? void 0 : options.minimal) ? [] : await this.sslMeta.list();
    const data = [];
    for (const confPath of confFiles) {
      ...
      if (!((_a = confData == null ? void 0 : confData.domain) == null ? void 0 : _a.length)) continue;
      data.push({
        confId,
        confPath,
        confName,
  ...
  async update(confId, options) {
    ...
    if (isDefault) return await __privateMethod(this, _WebSites_instances, newDefaultConf_fn).call(this, options);
    const validated = await this.nginx.validateAndSanitize(options);
    const siteConf = await this.nginx.readConf(path);
    await __privateMethod(this, _WebSites_instances, checkDomainsInUse_fn).call(this, validated == null ? void 0 : validated.domain, { exclude: (_a = siteConf == null ? void 0 : siteConf.domain) == null ? void 0 : _a.split(" ") });
  ...
  async rebuild(confId) {
    ...
    const siteConf = await this.nginx.readConf(path);
    if (isDefault) {
      return await __privateMethod(this, _WebSites_instances, newDefaultConf_fn).call(this, siteConf);
    }
  ...
  async rebuildAll() {
    ...
    const promises = sites.filter((site) => site.isActive).map((site) => this.rebuild(site == null ? void 0 : site.confId));
    await Promise.all(promises);
    await this.nginxReload();
  }
  async rebuildDefaultConf() {
    const defaultConfPath = fsPath__default.resolve(this.confDirPath, "_default.conf");
    if (!shelljs.test("-f", defaultConfPath)) {
      return await __privateMethod(this, _WebSites_instances, newDefaultConf_fn).call(this);
    }
  ...
  async installCert(domain) {
    var _a, _b;
    if ((_b = (_a = process == null ? void 0 : process.env) == null ? void 0 : _a.APP_ENV) == null ? void 0 : _b.startsWith("dev")) {
  ...
  async renewCert(domain) {
    ...
    if (domain.length < 1) throw new Error("Domain is missing");
    if (domain.length !== 1) throw new Error("Mulitple domains are not allowed");
    const ___domain = (_c = sanitizeDomains(domain)) == null ? void 0 : _c[0];
    const installed = await this.findAllCert();
    const domainCert = installed.filter((itm) => itm == null ? void 0 : itm.domains.includes(___domain));
  ...
  async findAllCert() {
    try {
      ...
      return __privateMethod(this, _WebSites_instances, unwindCerts_fn).call(this, installed, activeSites);
    } catch (error) {
      return [];
    }
  }
  ...
  async findOneCert(domain) {
    ...
    return (_a = [].concat(certs)) == null ? void 0 : _a[0];
  }
  async findSslDomains() {
    const installed = await this.sslAcme.listCertificates();
    const certs = __privateMethod(this, _WebSites_instances, unwindCerts_fn).call(this, installed);
  ...
  async findActiveSites() {
    ...
    return lo([]).concat(sites).filter((itm) => itm == null ? void 0 : itm.isActive).map((itm) => itm == null ? void 0 : itm.domain).flattenDeep().uniq().sort().value();
  }
  async findCertsExpiringIn(dayCount) {
    const installed = await this.findAllCert();
    const expiring = installed.filter((itm) => (itm == null ? void 0 : itm.daysLeft) <= (dayCount || -1));
    ...
    return validated.sort((a, b) => (a == null ? void 0 : a.daysLeft) - (b == null ? void 0 : b.daysLeft));
  }
...
parseHexaId_fn = function(id) {
  ...
  const file = fsPath__default.parse(path);
  return { path, file };
};
...
setConfData_fn = async function(args) {
  ...
  if (!(args == null ? void 0 : args.confPath)) {
    args.confPath = `${this.confDirPath}/${__privateMethod(this, _WebSites_instances, generateFilename_fn).call(this, args == null ? void 0 : args.domain)}${(args == null ? void 0 : args.isDumped) ? ".dump" : ".conf"}`;
  }
  if (!((_b = (_a = process == null ? void 0 : process.env) == null ? void 0 : _a.APP_ENV) == null ? void 0 : _b.startsWith("dev"))) {
    const dnsData = await __privateMethod(this, _WebSites_instances, dnsIpLookup_fn).call(this, args == null ? void 0 : args.domain);
    if (!(dnsData == null ? void 0 : dnsData.status)) throw `DNS A record for \`__**${args == null ? void 0 : args.domain}**__\` must be pointed to \`**${dnsData == null ? void 0 : dnsData.vpsIp}**\` instead of \`**${[].concat(dnsData == null ? void 0 : dnsData.dnsIps).join(",")}\`**.`;
  }
  const result = await this.nginx.writeConf(args == null ? void 0 : args.confPath, args);
  await this.nginxReload();
  return result;
};
newDefaultConf_fn = async function(args) {
...
dnsIpLookup_fn = async function(domain) {
  var _a;
  domain = (_a = sanitizeDomains(domain)) == null ? void 0 : _a[0];
...
checkDomainsInUse_fn = async function(domains, options) {
  ...
  const domainsInUse = lo([]).concat(availableSites).map((v) => sanitizeDomains(v == null ? void 0 : v.domain)).flattenDeep().uniq().sort().pull(...cleanArray(options == null ? void 0 : options.exclude)).value();
...
unwindCerts_fn = function(certs, activeSites) {
...
const assets = {
  "/CPanel_logo.svg": {
    ...
    "mtime": "2026-08-17T11:45:29.827Z",
    "size": 4754,
  ...
  "/_nuxt/B65QjWTN.js": {
    ...
    "etag": "\"6e-cVjp05OE9LUZhBfuHpd+SAaU3iw\"",
    "mtime": "2026-08-17T11:45:29.822Z",
    "size": 110,
    "path": "../public/_nuxt/B65QjWTN.js"
  },
  "/vps-logo1.svg": {
    "type": "image/svg+xml",
    "etag": "\"e908-dPk3avp20Ii8eNIRhnY5aj3Rycw\"",
  ...
  "/_nuxt/B-UzvF0K.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"1edf0-76wKO/ZhadlduWjlYh6kMxly2xc\"",
    "mtime": "2026-08-17T11:45:29.822Z",
    "size": 126448,
    "path": "../public/_nuxt/B-UzvF0K.js"
  },
  "/_nuxt/B1bxeG42.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"3477a-kdGHRllwiG6vntjWvV7mIJhSM0Y\"",
    "mtime": "2026-08-17T11:45:29.822Z",
    "size": 214906,
    "path": "../public/_nuxt/B1bxeG42.js"
  },
  "/_nuxt/BEfV-1oi.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"8678-9vGnEPjSzRFQ93eBpWBJkIk97t8\"",
    "mtime": "2026-08-17T11:45:29.822Z",
    "size": 34424,
    "path": "../public/_nuxt/BEfV-1oi.js"
  },
  "/_nuxt/BMNpjxUZ.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"9c1-5FywFPRJOO2GeosFFv41zL9zscA\"",
    "mtime": "2026-08-17T11:45:29.822Z",
    "size": 2497,
    "path": "../public/_nuxt/BMNpjxUZ.js"
  },
  "/_nuxt/BnUz8xkG.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"ea-D0ol0VYh9Zz+uEOWfC8gkG7Zwso\"",
    "mtime": "2026-08-17T11:45:29.822Z",
    "size": 234,
    "path": "../public/_nuxt/BnUz8xkG.js"
  },
  "/_nuxt/ByL2702e.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"256-LjJoE0q6FZ4ooxHCg9xUdNX3Pwo\"",
    "mtime": "2026-08-17T11:45:29.822Z",
    "size": 598,
    "path": "../public/_nuxt/ByL2702e.js"
  },
  "/_nuxt/C3Yqi9iI.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"1ffb-/2Yj359pHwLn+6/Ok7ql8gTiSYA\"",
    "mtime": "2026-08-17T11:45:29.822Z",
    "size": 8187,
    "path": "../public/_nuxt/C3Yqi9iI.js"
  },
  "/_nuxt/C450Ubem.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"d2-HLohsxV/946ixjX3JhnDoqpDSUM\"",
    "mtime": "2026-08-17T11:45:29.822Z",
    "size": 210,
    "path": "../public/_nuxt/C450Ubem.js"
  },
  "/_nuxt/C80tFsz-.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"f7d-RhW//XU7JX18unQc5N40AZrZXfI\"",
    "mtime": "2026-08-17T11:45:29.822Z",
    "size": 3965,
    "path": "../public/_nuxt/C80tFsz-.js"
  },
  "/_nuxt/CDS8J87L.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"137-ruphPAOdlJgYgZEo75aEFl77xWc\"",
    "mtime": "2026-08-17T11:45:29.822Z",
    "size": 311,
    "path": "../public/_nuxt/CDS8J87L.js"
  },
  "/_nuxt/CE1Hsin7.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"127ce-x3YQ7bdPSEpCZOfKX/H2tbiNn/A\"",
    "mtime": "2026-08-17T11:45:29.823Z",
    "size": 75726,
    "path": "../public/_nuxt/CE1Hsin7.js"
  },
  "/_nuxt/CZQnppu7.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"4b-YUM9nHZyZFMETlwwwTY7VOOmBx0\"",
    "mtime": "2026-08-17T11:45:29.823Z",
    "size": 75,
    "path": "../public/_nuxt/CZQnppu7.js"
  },
  "/_nuxt/Cuke60oi.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"32e9-R17CJnUh94QJTqi7qelT958gFZE\"",
    "mtime": "2026-08-17T11:45:29.822Z",
    "size": 13033,
    "path": "../public/_nuxt/Cuke60oi.js"
  },
  "/_nuxt/CrsLQSfw.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"41e1-6RevG9a4fmW0iSUNYdkII9raefY\"",
    "mtime": "2026-08-17T11:45:29.822Z",
    "size": 16865,
    "path": "../public/_nuxt/CrsLQSfw.js"
  },
  "/_nuxt/D3v7_4RN.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"16c-kXu6IKRwHq+oaHv/o4ORrw/kyIs\"",
    "mtime": "2026-08-17T11:45:29.822Z",
    "size": 364,
    "path": "../public/_nuxt/D3v7_4RN.js"
  },
  "/_nuxt/D4iPArhg.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"182-9FtKOxV6y2wqpzZw95SVmv5p8xQ\"",
    "mtime": "2026-08-17T11:45:29.822Z",
    "size": 386,
    "path": "../public/_nuxt/D4iPArhg.js"
  },
  "/_nuxt/D60cATJ0.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"e27-WmfM2sxhDCWsjyvstNCPRDJZ67o\"",
    "mtime": "2026-08-17T11:45:29.823Z",
    "size": 3623,
    "path": "../public/_nuxt/D60cATJ0.js"
  },
  "/_nuxt/DebAK_Mb.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"259-B98OFLoAB+llZ5bLHwojILbZojQ\"",
    "mtime": "2026-08-17T11:45:29.823Z",
    "size": 601,
    "path": "../public/_nuxt/DebAK_Mb.js"
  },
  "/_nuxt/DgWS9iMI.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"276-mVbrcNcNxTz4dPFA1yQIPTpFt8w\"",
    "mtime": "2026-08-17T11:45:29.823Z",
    "size": 630,
    "path": "../public/_nuxt/DgWS9iMI.js"
  },
  "/_nuxt/DJMrDfbJ.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"3fc31-jnvyW8nyKUCcMZ4lR+gQQt+cIgo\"",
    "mtime": "2026-08-17T11:45:29.823Z",
    "size": 261169,
    "path": "../public/_nuxt/DJMrDfbJ.js"
  },
  "/_nuxt/DltzwZ1y.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"4fe-sMACERvdk0eXlxhfd8Po8B5vQ6Q\"",
    "mtime": "2026-08-17T11:45:29.823Z",
    "size": 1278,
    "path": "../public/_nuxt/DltzwZ1y.js"
  },
  "/_nuxt/Dx28W3t-.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"1e9-UWecOrQa0FRqfKKVue60tMV6k9w\"",
    "mtime": "2026-08-17T11:45:29.823Z",
    "size": 489,
    "path": "../public/_nuxt/Dx28W3t-.js"
  },
  "/_nuxt/VP702KAl.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"536b-trMPURI3dZcJf5pLhYQt65pAaqg\"",
    "mtime": "2026-08-17T11:45:29.823Z",
    "size": 21355,
    "path": "../public/_nuxt/VP702KAl.js"
  },
  "/_nuxt/RgbRUjhJ.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"14c-V6YPsTbpqxVq0XOi7O3Hwpt+Voc\"",
    "mtime": "2026-08-17T11:45:29.823Z",
    "size": 332,
    "path": "../public/_nuxt/RgbRUjhJ.js"
  },
  "/_nuxt/entry.CBZh8sOE.css": {
    "type": "text/css; charset=utf-8",
    "etag": "\"371d-5UhlyPiq1wBgvqVCq9MZc7oxIwA\"",
    "mtime": "2026-08-17T11:45:29.823Z",
    "size": 14109,
    "path": "../public/_nuxt/entry.CBZh8sOE.css"
  },
  "/_nuxt/nbeONyMQ.js": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"1784-D/Q5AeifptTI036+Chzs128i7Fc\"",
    "mtime": "2026-08-17T11:45:29.823Z",
    "size": 6020,
    "path": "../public/_nuxt/nbeONyMQ.js"
  },
  "/_nuxt/builds/latest.json": {
    "type": "application/json",
    "etag": "\"47-iFpgcOQ4xxuNGccewqe8O2/xVY4\"",
    "mtime": "2026-08-17T11:45:29.799Z",
    "size": 71,
    "path": "../public/_nuxt/builds/latest.json"
  },
  "/_nuxt/builds/meta/6159790e-6354-44a9-9398-4369d049c84c.json": {
    "type": "application/json",
    "etag": "\"58-W0SHzUlR0DUkj0I5dTXuFbC2F3g\"",
    "mtime": "2026-08-17T11:45:29.795Z",
    "size": 88,
    "path": "../public/_nuxt/builds/meta/6159790e-6354-44a9-9398-4369d049c84c.json"
  }
};
...
const _acHVEW = eventHandler((event) => {
  ...
  event.cronResponse = (data, err) => {
    ...
    const errorMessage = isValidString(err) ? err : (err == null ? void 0 : err.message) || (err == null ? void 0 : err.statusMessage) || (err == null ? void 0 : err.statusText) || "oops, something went wrong";
...
`;
  ...
  event.errorResponse = (err, statusCode, statusText) => {
    ...
    const errorMessage = isValidString(err) ? err : (err == null ? void 0 : err.message) || (err == null ? void 0 : err.statusMessage) || (err == null ? void 0 : err.statusText) || statusText || err || "oops, something went wrong";
    ...
    setResponseHeaders(event, { "Content-Type": "application/json" });
    setResponseStatus(event, (err == null ? void 0 : err.statusCode) || statusCode || 400, cleanStatusText(statusMessage));
...
const validateAdminUser = (inps, envs) => {
  if ((inps == null ? void 0 : inps.user) !== (envs == null ? void 0 : envs.user)) throw new Error("Invalid User");
  const isPasswordCorrect = comparePassword(inps == null ? void 0 : inps.pass, envs == null ? void 0 : envs.pass);
...
const _lazy_ZokV0m = () => import('../routes/api/public/daily-crons.get.mjs');
const _lazy_Q71Fs6 = () => import('../routes/api/public/hourly-crons.get.mjs');
const _lazy_2x4JVV = () => import('../routes/api/public/minute-crons.get.mjs');
const _lazy_L0Q02S = () => import('../routes/api/public/monthly-crons.get.mjs');
const _lazy_RG8v41 = () => import('../routes/api/public/weekly-crons.get.mjs');
const _lazy_oX_vwz = () => import('../routes/api/public/yearly-crons.get.mjs');
const _lazy_kMkE9V = () => import('../routes/api/verify.mjs');
const _lazy_apDHPU = () => import('../routes/renderer.mjs').then(function (n) { return n.r; });
...
const handlers = [
  ...
  { route: '/api/public/daily-crons', handler: _lazy_ZokV0m, lazy: true, middleware: false, method: "get" },
  { route: '/api/public/hourly-crons', handler: _lazy_Q71Fs6, lazy: true, middleware: false, method: "get" },
  { route: '/api/public/minute-crons', handler: _lazy_2x4JVV, lazy: true, middleware: false, method: "get" },
  { route: '/api/public/monthly-crons', handler: _lazy_L0Q02S, lazy: true, middleware: false, method: "get" },
  { route: '/api/public/weekly-crons', handler: _lazy_RG8v41, lazy: true, middleware: false, method: "get" },
  { route: '/api/public/yearly-crons', handler: _lazy_oX_vwz, lazy: true, middleware: false, method: "get" },
  { route: '/api/verify', handler: _lazy_kMkE9V, lazy: true, middleware: false, method: undefined },
  { route: '/__nuxt_error', handler: _lazy_apDHPU, lazy: true, middleware: false, method: undefined },
  { route: '/__nuxt_island/**', handler: _SxA8c9, lazy: false, middleware: false, method: undefined },
  { route: '/**', handler: _lazy_apDHPU, lazy: true, middleware: false, method: undefined }
];
...
export { $fetch as $, AccountJson as A, isScriptProtocol as B, joinURL as C, withQuery as D, defuFn as E, sanitizeStatusCode as F, getContext as G, defu as H, createHooks as I, executeAsync as J, parseQuery as K, withTrailingSlash as L, withoutTrailingSlash as M, nodeServer as N, SslMeta as S, WebSites as W, getRouterParams as a, getQuery as b, runCronJobTask as c, sudoExec as d, eventHandler as e, fetchApi as f, getHeaders as g, getRequestProtocol as h, cleanArray as i, isInstalled as j, useAppConfig as k, joinRelativeURL as l, encodePath as m, defineRenderHandler as n, createError$1 as o, getRouteRules as p, getResponseStatusText as q, readBody as r, sendEmailNow as s, getResponseStatus as t, useRuntimeConfig as u, useNitroApp as v, parseURL as w, decodePath as x, klona as y, hasProtocol as z };
