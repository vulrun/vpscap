...
/**
 * Returns a comparator function to sort objects based on the provided options.
 * @param {Object} options - Should include sortKey, sortOrder, numberFirst, and stringFirst.
 * @returns {Function}
 */
export function sortData({ sortKey, sortOrder, numberFirst, stringFirst }) {
  function validValues(values, input, defaultVal) {
    if (!Array.isArray(values)) return defaultVal;
    return values.includes(input) ? input : defaultVal;
  }

  // Utility type-checking methods
  ...
  numberFirst = validValues([true, false], numberFirst, true);
  stringFirst = validValues([true, false], stringFirst, null);
  ...
  return (a, b) => {
    const aValue = sortKey ? a[sortKey] : a;
    const bValue = sortKey ? b[sortKey] : b;

    // Precedence between string and number values
    if (isString(aValue) && isNumber(bValue)) return 1 * sortPriority;
    if (isNumber(aValue) && isString(bValue)) return -1 * sortPriority;
    // Compare booleans
    ...
    if (isString(aValue) && isString(bValue)) {
      const isAAlpha = /^[A-Za-z]/.test(aValue);
      const isBAlpha = /^[A-Za-z]/.test(bValue);
      if (isAAlpha && !isBAlpha) return 1 * sortPriority;
      if (!isAAlpha && isBAlpha) return -1 * sortPriority;
